# Model Routing · GPT-Image-2.5 官方双模型选型指南

> 何时读：组装完 prompt、调用 imagegen.sh 之前——决定 `IMAGE_MODEL`（或 `--quality`）。
> 说明：`gpt-image-2.5-flare` 与 `gpt-image-2.5-sunburst` 是 OpenAI 官方模型 ID。第三方 OpenAI 兼容端点可能尚未同步开放，使用前检查端点模型列表。
...
|---|---|---|
| OpenAI 官方定位 | 多数应用的默认选择；快速、高质量的日常生成 | 最强图像生成与编辑模型；更强的精细编辑控制 |
...
Flare 相比 GPT-Image-2 的官方发布口径是质量更高、延迟降低 50%；Sunburst 以更长生成时间换取复杂创意与编辑任务中的更高精度。实际延迟仍会受尺寸、质量档、负载与端点影响。
